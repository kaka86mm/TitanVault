...
| **titanvault-ops** | 硬件与架构知识库（被动知识） | 硬件问题、GPU/端口/配置查询 |

**会议 skill** 提供两条路径：*完整模式*（Aham Voice：转写 + 说话人分离 + AI 纪要 + 情绪）或*快速模式*（SenseVoice：快速出纯文字稿）。入库 skill 自动识别 PDF，走 MinerU 解析（保留表格/公式）后再 embedding。
