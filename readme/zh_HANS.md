...
| Skill | 做什么 | 触发语 |
|---|---|---|
| **mozin-ops** | 运维与排障工作站（健康检查、自愈、备份、报告） | "check status"、"heal"、"disk full" |
| **mozin-meeting** | 把会议录音变成结构化 markdown 纪要 | "处理这个会议录音"、"转写录音"、"生成会议纪要" |
| **mozin-ingest** | 把任意 URL / PDF / 文本存入知识库，并可就内容提问 | "把这个链接存进知识库"、"存这个 PDF"、"这篇讲了什么" |
...
├── ops/                      # 场景 skill（meeting、ingest、mozin-ops）
