...
Hermes 内置**面向场景的 skill**——不是 API 封装，而是把多个服务串起来达成一个用户目标的端到端工作流。每个 skill 遵循统一模式：权限矩阵 + 确定性脚本 + 失败模式表 + 反模式黑名单。

| Skill | 做什么 | 触发语 |
|---|---|---|
| **titanvault-ops** | 运维与排障工作站（健康检查、自愈、备份、报告） | "check status"、"heal"、"disk full" |
| **titanvault-meeting** | 把会议录音变成结构化 markdown 纪要 | "处理这个会议录音"、"转写录音"、"生成会议纪要" |
| **titanvault-ingest** | 把任意 URL / PDF / 文本存入知识库，并可就内容提问 | "把这个链接存进知识库"、"存这个 PDF"、"这篇讲了什么" |
| **titanvault-research** | 深度研究 agent——QUEST-9B 自主搜索、阅读、生成带引用的研究报告 | "深度研究 XX"、"全面分析一下 XX"、"调研 XX" |
| **titanvault-knowledge** | 硬件与架构知识库（被动知识） | 硬件问题、GPU/端口/配置查询 |

**会议 skill** 提供两条路径：*完整模式*（Aham Voice：转写 + 说话人分离 + AI 纪要 + 情绪）或*快速模式*（SenseVoice：快速出纯文字稿）。入库 skill 自动识别 PDF，走 MinerU 解析（保留表格/公式）后再 embedding。研究 skill 用 [QUEST-9B](https://osu-nlp-group.github.io/QUEST/)（OSU NLP 的深度研究模型）在 ReAct 循环中驱动 SearXNG + 网页抓取，完成后串联入库做 RAG。

还内置两个用于构建和优化更多 skill 的**元 skill**：

| 元 skill | 角色 | 来源 |
|---|---|---|
| **女娲 (Nuwa)** | 蒸馏人物/主题的思维框架，生成可运行的 skill | [alchaincyf/nuwa-skill](https://github.com/alchaincyf/nuwa-skill) |
| **达尔文 (Darwin)** | 用 9 维 rubric（SkillLens）评估并优化任意 skill，含爬山迭代 | [alchaincyf/darwin-skill](https://github.com/alchaincyf/darwin-skill) |

> 会议与入库 skill 已用达尔文的 9 维 rubric 评估，并在真机上端到端实测通过（full-test，非干跑）：会议转写、URL 入库、RAG 问答均通过。
...
├── config/                   # 模板（.env、Caddy、LiteLLM、Hermes）
│   └── hermes/skills/        # Hermes skills（运维知识 + 女娲/达尔文元 skill）
├── ops/                      # 场景 skill（meeting、ingest、titanvault-ops）
├── presets/                  # minimal / standard / full
├── hardware/                 # Strix Halo 专属参数
├── models/                   # 模型清单 + 下载配置
├── scripts/                  # 安装自动化（模型、kuma、notebook、...）
└── docs/                     # 文档
```
