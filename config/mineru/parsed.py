...
def get_parsed_content(
    ...
    try:
        ...
        if _is_missing_object_error(e):
            # fallback: markdown_page/popo 等变体缺失时, 回退到整篇 {stem}.md
            # (mineru-api 默认只输出整篇 markdown, 不生成按页 _pages.md)
            fallback_path = f"{_artifact_stem(file)}.md"
            try:
                return _read_minio_object(mds_bucket, fallback_path).decode("utf-8")
            except Exception:
